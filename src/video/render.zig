...
pub const Renderer = struct {
    ...
    pub fn clear(self: Renderer) !void {
        if (c.SDL_RenderClear(self.handle) == false) {
    ...
    pub fn present(self: Renderer) !void {
        if (c.SDL_RenderPresent(self.handle) == false) {
            return error.SDLError;
        }
    }
    ...
    pub fn setColor(self: Renderer, color: pixels.Color) !void {
        if (c.SDL_SetRenderDrawColor(self.handle, color.r, color.g, color.b, color.a) == false) {
            return error.SDLError;
        }
    }
    ...
    pub fn drawLine(self: Renderer, x1: f32, y1: f32, x2: f32, y2: f32) !void {
        if (c.SDL_RenderLine(self.handle, x1, y1, x2, y2) == false) {
            return error.SDLError;
        }
    }
    ...
    pub fn fillRect(self: Renderer, x: f32, y: f32, w: f32, h: f32) !void {
        ...
        if (c.SDL_RenderFillRect(self.handle, &fill_rect) == false) {
            return error.SDLError;
        }
    }
    ...
    pub fn drawRect(self: Renderer, x: f32, y: f32, w: f32, h: f32) !void {
        ...
        if (c.SDL_RenderRect(self.handle, &draw_rect) == false) {
            return error.SDLError;
        }
    }
    ...
    pub fn renderText(self: Renderer, x: f32, y: f32, text: []const u8) !void {
        if (c.SDL_RenderDebugText(self.handle, x, y, text.ptr) == false) {
            return error.SDLError;
        }
    }
};
